package com.apptech.myapplication.ui.order.order_status;
...
import com.apptech.myapplication.adapter.OrderStatusAdapter;
import com.apptech.myapplication.databinding.OrderStatusFragmentBinding;
...
public class OrderStatusFragment extends Fragment implements View.OnClickListener {
    ...
    List<com.apptech.myapplication.list.OrderStatusList> orderStatusLists = new ArrayList<>();
                ...
                if(response.isSuccessful()){
                    if(!response.body().getError()){
                        ...
                        for (int i=0 ; i < response.body().getList().size() ; i++){
                            com.apptech.myapplication.modal.order_statusList.List l = response.body().getList().get(i);
                            if(type.toUpperCase().trim().equalsIgnoreCase(l.getstatus().toUpperCase().trim())){
                                orderStatusLists.add(new com.apptech.myapplication.list.OrderStatusList(
                                        l.getId()
                                        ,l.getProductId()
                                        ,l.getModelName()
                                        ,l.getProductName()
